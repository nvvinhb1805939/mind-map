...
import { getEditingMode, getLastSelectedElement, onDeleteElements } from 'src/utils/mindMap';
...
export const EdgeEditing = memo(({ selected, copied }) => {
  ...
  const lastSelectedEdge = useMemo(
    () => getLastSelectedElement(selected, EDIT_MODES.EDGE_EDITING),
    [selected]
  );
  ...
  const copyEdgeFormat = () => {
    dispatch(
      copyFormat({
        ...
        style: { stroke: DEFAULT_EDGE_COLOR, ...lastSelectedEdge.element?.style },
      })
    );
  ...
  return (
    <Stack direction="row" justifyContent="space-between" gap={1}>
      <ColorPicker
        id={`color-picker-edge-${lastSelectedEdge.element.id}`}
        onChangeComplete={onChangeComplete}
        initialColor={lastSelectedEdge.element?.style?.stroke || DEFAULT_EDGE_COLOR}
